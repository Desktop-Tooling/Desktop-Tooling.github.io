...
export function DemoIdentity(props: {
  crumbs: Crumb[]
  repoUrl: string
  repoLabel?: string
}) {
  return (
    <div class="mb-8 space-y-3">
      <nav class="flex flex-wrap items-center gap-2 text-sm text-muted-foreground" aria-label="Breadcrumb">
        <A href="/" class="text-foreground no-underline hover:underline">
          Desktop Tooling
        </A>
        <For each={props.crumbs}>
          {(c) => (
            <>
              ...
              <Show
                when={c.href}
                fallback={<span class="text-foreground">{c.label}</span>}
              >
                <A href={c.href!} class="text-foreground no-underline hover:underline">
                  {c.label}
                </A>
              </Show>
            </>
      ...
      <div>
        <a
          href={props.repoUrl}
          class="inline-flex items-center gap-2 text-sm font-medium text-foreground no-underline hover:underline"
          rel="noreferrer"
          target="_blank"
        >
          <GitHubMark />
          <span>{props.repoLabel ?? "GitHub repository"}</span>
        </a>
      </div>
    </div>
  )
